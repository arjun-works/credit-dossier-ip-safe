...
class Settings:
    ...
    def postgres_kwargs(self, database: str | None = None) -> dict[str, object]:
        ...
        if conn_url and ("postgresql" in conn_url or "postgres://" in conn_url) and not database:
            if conn_url.startswith("postgresql+psycopg://"):
                conn_url = "postgresql://" + conn_url[len("postgresql+psycopg://"):]
            elif conn_url.startswith("postgres://"):
                conn_url = "postgresql://" + conn_url[len("postgres://"):]
            return {"conninfo": conn_url}
