...
def ensure_database() -> None:
    ...
    # When using a remote cloud database (e.g. Neon, Supabase), the database is already provisioned
    conn_url = os.getenv("MCP_DATABASE_URL") or os.getenv("DATABASE_URL")
    if conn_url and ("neon.tech" in conn_url or "supabase" in conn_url or "aws" in conn_url):
        return
    try:
        with psycopg.connect(
        ...
        logger.warning(
            "Could not connect to default database to ensure %s exists (%s). "
            "Assuming target database is already created.",
            settings.postgres_database,
            e,
        )
